...
Use only your own authorized account. Protect `.proxykey`, generated settings/env files and profile configurations; never paste their contents into issues. The proxy is loopback-only and bearer-authenticated. Remote CAPTCHA JavaScript is not OS-isolated: `createDom` without injected local test fixtures is disabled by default, and a bare proxy started outside the kit must opt in explicitly with `ZCODE_PROXY_ALLOW_UNSANDBOXED_CAPTCHA=1`. The kit's managed proxy service sets that opt-in itself (`proxyEnv` applies it to every kit-spawned proxy), so the bundled solver automatically handles upstream captcha challenges in-process. Identifiable asynchronous CAPTCHA-script errors no longer terminate headless `serve`; other unhandled errors remain fatal. This executes vendor challenge JS without an OS sandbox, does not override account or quota restrictions, and cannot guarantee that every challenge succeeds. Trial-claiming and off-peak automation are disabled by default. Read [SECURITY.md](SECURITY.md) before changing settings or exposing any endpoint.
