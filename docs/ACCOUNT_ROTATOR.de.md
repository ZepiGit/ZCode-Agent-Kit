...
| `exhausted` | Im Cooldown nach einem Kontingentfehler, bis zur angezeigten Zeit. Bevor die Abklingzeit beginnt, wiederholt das Kit dieselbe Anmeldung nach einem wachsenden Zeitplan (Standard nach 1s, 4s, 10s, 20s und 30s — ein Request kann etwa eine Minute warten); das Gateway kann die Wiederholung dann aus einem anderen Kontingentpaket bedienen, das noch Guthaben hat (etwa ein Event-Guthaben neben einem leeren Tagespaket). Der Zeitplan gilt auch für Installationen mit nur einem Konto. Im Account-Rotator-Modus gilt die Abklingzeit auch, wenn die Wiederholungen deaktiviert sind, verweigert werden oder mit einem anderen Fehler scheitern; nur das Single-Account-Memo wartet auf nachgewiesene Erschöpfung. Zeitplan anpassen oder deaktivieren mit `ZCODE_PROXY_QUOTA_RETRY_DELAYS_MS` (kommagetrennte Millisekunden; `0` bedeutet einen sofortigen Versuch, `off` deaktiviert; max. 6 Einträge, je maximal 60s; nur Dezimalziffern — ungültige Werte fallen auf den Standardzeitplan zurück; Proxy-Start/Neustart reicht die Variable durch). |
