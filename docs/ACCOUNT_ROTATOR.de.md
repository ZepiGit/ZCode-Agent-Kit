...
| `exhausted` | Im Cooldown nach einem Kontingentfehler, bis zur angezeigten Zeit. Bevor die Abklingzeit beginnt, wiederholt das Kit dieselbe Anmeldung nach einem wachsenden Zeitplan (Standard nach 1s, 4s, 10s, 20s und 30s) — das Gateway kann die Wiederholung dann aus einem anderen Kontingentpaket bedienen, das noch Guthaben hat (etwa ein Event-Guthaben neben einem leeren Tagespaket). Die Abklingzeit gilt nur, wenn die Wiederholungen selbst Kontingenterschöpfung melden. Der Zeitplan lässt sich mit `ZCODE_PROXY_QUOTA_RETRY_DELAYS_MS` anpassen (kommagetrennte Millisekunden, z. B. `0` oder `off` zum Deaktivieren, max. 6 Einträge, je maximal 60s). |
