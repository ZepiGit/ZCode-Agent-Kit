...
| `exhausted` | In cooldown after a quota error, until the shown time. Before the cooldown starts, the kit retries the same account on a growing schedule (default after 1s, 4s, 10s, 20s and 30s — a request can be held for about a minute); the gateway may serve a retry from another balance package that still has quota (e.g. an event grant next to an empty daily package). The schedule applies to single-account installs too. In Account Rotator mode the cooldown also applies when the retries are disabled, refused, or failed with a different error; only the single-account retry memo waits for proven exhaustion. Tune or disable with `ZCODE_PROXY_QUOTA_RETRY_DELAYS_MS` (comma-separated milliseconds; `0` means one immediate retry, `off` disables; max 6 entries, each capped at 60s; decimal digits only — invalid values fall back to the default schedule; proxy start/restart passes it through). |
