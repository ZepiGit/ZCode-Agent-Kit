...
直接启动 OMP，例如 `omp --model zcode/glm-5.3-flash --thinking low`，不要通过 `zcode-kit run` 启动。设置过程固定原生 Node/Bun；自启动预检查使用新的子进程，不将 Kit 模块导入 OMP。子进程最长运行 120 秒，并报告不含秘密信息的错误类别。修复原因后，等待该会话的 60 秒冷却时间再重试；可在同一会话中恢复。若运行时位置已变更，请重新运行 `zcode-kit setup --harness auto` 并重新加载扩展。不会终止占用端口的未知进程。仅有代理健康或设置成功并不能证明模型回复能够完成。
...
**Flash：**`glm-5.3-flash` 始终启用 thinking。显式禁用 thinking 会被规范化为 `low`；显式选择的 `high` 和 `max` 保持不变。对于 Flash，Anthropic 格式的低级别 thinking 预算为 `8000` tokens（而非上方通用示例的 `2048`），并额外预留回答的输出空间；OpenAI 格式使用 `reasoning_effort: low`。不要仅凭较高的 thinking 强度就认定发生了卡死。通过直接代理调用和 Claude Code 已成功完成 Flash 回复；这不代表所有 harness 均已通过验收。
