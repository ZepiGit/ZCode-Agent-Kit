...
The key lives only locally (`<clone>/.proxykey`) and is generated by setup.mjs.

## Set up automatically by `zcode-kit setup` (only for detected harnesses)

setup.mjs detects which harnesses are installed and sets things up **only for
those**. A user with only OMP gets no Claude/Codex artifacts whatsoever (no
generated files either).
...
| Goose | `%APPDATA%/Block/goose/config/custom_providers/zcode.json` | credential via documented `auth.command` helper (kit key resolver, no shell) |
...
|---|---|---|
| Claude Code | `bin\zcode-claude.cmd` | starts the proxy on demand and calls `claude --settings <clone>\generated\claude-zcode-settings.json` (CLI settings override user settings.json; your normal `claude` keeps running unchanged) |
| Codex CLI | `bin\zcode-codex.cmd` | sets `CODEX_HOME=<clone>\generated\codex-home` + `ZCODE_PROXY_KEY` and starts the proxy on demand; your normal `codex` and `~/.codex` stay untouched |
...
```json
{
  "mcpServers": {
    "zcode-harness": {
      "type": "stdio",
      "command": "node",
      "args": ["<absolute clone path>/mcp/zcode-harness-mcp/dist/index.js", "--stdio"]
    }
  }
}
```

The bridge drives the **real installed ZCode Desktop** (app-server protocol:
sessions, turns, tasks). Limitations: the desktop must be running (it solves
the Z.AI captchas); reasoning levels via the bridge catalog are `low/high/max`;
the desktop plan catalog lists GLM-5.3/GLM-5-Turbo — GLM-5.3-Flash goes through
the proxy path, not the desktop bridge. Details:
[mcp/zcode-harness-mcp/README.md](../mcp/zcode-harness-mcp/README.md).

## Quota & error modes

- `GET /quota` (authenticated) shows the token buckets per model.
- Quota exhausted → HTTP 400 `[1005] exceed quota limit` (not retryable — wait for the reset, GLM-5.3: daily at 18:00 local time).
- `[3007] captcha verify failed` → gateway anti-abuse after intense retrying; take a pause.
- `401 start_plan_jwt_invalid` → renew the desktop login ([README.md](../README.md) → "Login renewal").
- Codex path: occasional cosmetic error `OutputTextDelta without active item` in the responses handler — the result stays correct (known cosmetic issue).
