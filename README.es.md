...
- JavaScript CAPTCHA remoto no tiene sandbox del sistema operativo y está deshabilitado por defecto fuera del kit: un proxy iniciado manualmente requiere el consentimiento explícito `ZCODE_PROXY_ALLOW_UNSANDBOXED_CAPTCHA=1`. El servicio gestionado por el kit activa ese consentimiento a través de `proxyEnv` y resuelve los desafíos CAPTCHA automáticamente (en proceso, sin sandbox del SO); esto no elude las restricciones del proveedor.
