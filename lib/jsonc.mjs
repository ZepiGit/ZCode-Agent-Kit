...
function findTopLevelValueSpan(text, key) {
  ...
  while (i < text.length) {
    ...
    const next = text[i + 1];
    if (mode === "code") {
      // AUD-002: the top-level key scanner MUST run before the generic
      // string transition — otherwise every top-level key enters string
      // mode here and the scanner below is unreachable, so setTopLevelKey
      // always inserts (duplicate keys) instead of replacing.
      if (depth === 1 && c === '"') {
