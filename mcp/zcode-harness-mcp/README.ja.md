...
- テスト：決定論的フィクスチャハーネスに対する 7 つのユニットテスト +
  20 の統合/ロバストネステスト + 実インストールに対する 5 つのライブ
  テスト（[`TEST_REPORT.md`](TEST_REPORT.md)）。
...
npm run test:live # 実インストールに対するライブテスト： $env:LIVE_TEST="1"; $env:LIVE_WORKSPACE="C:\..."; $env:LIVE_DATA_DIR="C:\..."
```
...
| `--http --port N --host H` | Streamable-HTTP モード（デフォルト 127.0.0.1:3322） |
...
| `--interaction-allowlist "Bash,Read"` | ポリシー `allowlist` 用のプレフィックス許可リスト |
