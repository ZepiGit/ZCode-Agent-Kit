...
export async function readArtifact(workspace, candidate, offset, length, maxBytes) {
    ...
    try {
        ...
        if (before.size > maxBytes)
            throw new Error(`ARTIFACT_TOO_LARGE: maximum ${maxBytes} bytes`);
        const verified = resolveInsideWorkspace(workspace, candidate);
        const named = await fs.promises.stat(verified);
        if (verified !== abs || named.dev !== before.dev || named.ino !== before.ino)
            throw new Error("artifact path changed while opening");
        ...
        while (position <= maxBytes) {
            ...
            if (to > from)
                parts.push(Buffer.from(chunk.subarray(from - position, to - position)));
            position += bytesRead;
        }
        const after = await handle.stat();
        if (position !== before.size || before.size !== after.size || before.mtimeMs !== after.mtimeMs)
