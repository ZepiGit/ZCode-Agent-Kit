...
function isLoopbackHost(host) {
    ...
    if (LOOPBACK_HOSTS.has(h) || LOOPBACK_HOSTS.has(`[${h}]`))
        return true;
...
export function parseConfig(argv) {
    ...
    for (let i = 0; i < args.length; i += 1) {
        ...
        switch (a) {
            ...
            default:
                if (a.startsWith("-")) {
                    throw new Error(`unknown option "${a}" — refusing to start with a possibly-misspelled flag ` +
    ...
    if (config.transport === "http") {
        if (!isLoopbackHost(config.host)) {
            throw new Error(`refusing to bind HTTP transport to non-loopback host "${config.host}" — ` +
                `the bridge exposes desktop control and only supports 127.0.0.1 / localhost / ::1`);
        }
        if (!config.httpKey) {
            throw new Error("HTTP transport requires --http-key (or ZCODE_HARNESS_HTTP_KEY) — " +
