...
export class ZcodeConnection {
  ...
  private async spawnChild(): Promise<void> {
    ...
    const exitMessage = (code: number | null, signal: string | null): string =>
      `harness exited (code=${code}, signal=${signal})${providerConfigMissing
    ...
    child.stderr.on("data", (chunk: string) => {
      ...
      if (text.includes("无法定位 CLI ZCode Built-in Provider Config")) providerConfigMissing = true;
    ...
    child.on("exit", (code, signal) => {
      ...
      this.failAllPending(
        new ZcodeConnectionError("HARNESS_EXITED", exitMessage(code, signal))
      );
      this.child = null;
