/**
 * CapabilityRegistry — single source of truth for what the bridge exposes
 * and what the underlying harness supports. Availability was verified live
 * against the local harness (see CAPABILITY_MATRIX.md for evidence and
 * fingerprint); statuses distinguish availability, implementation and
 * verification.
 */
...
export type Implementation = "implemented" | "partial" | "missing" | "not_applicable";
export type Verification = "live_verified" | "fixture_verified" | "untested" | "blocked_by_environment";

export interface Capability {
  id: string;
  title: string;
  description: string;
  /** Backend access path (native method or bridge-side logic). */
  backend: string;
  direction: "client_call" | "server_callback" | "event";
  availability: Availability;
  implementation: Implementation;
  verification: Verification;
  /** Public MCP tool that exposes this capability, if any. */
  mcpTool: string | null;
  /** Evidence note (live probe result, fixture test id, etc.). */
  evidence: string;
}
...
export function capabilitiesForMcp(): Array<Record<string, unknown>> {
