...
export const TERMINAL_STATES: TaskState[] = ["completed", "failed", "cancelled", "interrupted"];
...
export class TaskManager {
  ...
  restore(): number {
    ...
    for (const f of files) {
      ...
      if (!rec || rec.schemaVersion !== 1) continue;
  ...
  async startTask(params: StartTaskParams): Promise<TaskRecord> {
    ...
    if (!requestedReadOnly) {
      const conflicting = this.list().find(
        (t) => t.workspacePath === params.workspacePath && !TERMINAL_STATES.includes(t.state) && !t.readOnly
      );
      if (conflicting) {
        throw new TaskError(
          "WORKSPACE_BUSY",
  ...
  private scheduleStart(t: TaskInternal, params: StartTaskParams): void {
    const isActive = (r: TaskRecord): boolean =>
      ["starting", "running", "waiting_for_input", "waiting_for_approval", "cancelling"].includes(r.state);
  ...
  async addInput(taskId: string, content: string): Promise<TaskRecord> {
    ...
    if (rec.state === "cancelling") throw new TaskError("CANCELLING", "task is being cancelled");
  ...
  async cancel(taskId: string): Promise<TaskRecord> {
    ...
    if (!t) throw new TaskError("TASK_NOT_FOUND", `unknown task: ${taskId}`);
    const rec = t.record;
    if (TERMINAL_STATES.includes(rec.state)) return rec;
  ...
  async buildResult(taskId: string): Promise<TaskResultV1> {
    ...
    let completeness: TaskResultV1["completeness"] = { status: "full", explanation: "turn stream captured via session events" };
    if (!responseText && rec.sessionId) {
      try {
        ...
        const entries = Array.isArray(transcript?.entries) ? (transcript.entries as Array<Record<string, unknown>>) : [];
