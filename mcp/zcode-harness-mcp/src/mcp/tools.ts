...
export function buildTools(ctx: ToolContext): ToolSpec[] {
  ...
  tools.push({
    ...
    handler: async (args) => {
      const { workspacePath } = ws(ctx, args);
      const state = await ctx.runtime.callForWorkspace<Record<string, unknown>>("workspace/readState", workspacePath, {});
  ...
  tools.push({
    ...
    description:
      "Select the model (providerId/modelId from the live catalog), optional reasoning level and/or permission mode for a session or a workspace default. Returns requested AND effective values after read-back verification. GLM-5.3-Flash is only used when present in the actual catalog; otherwise a clear error with the catalog is returned.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  tools.push({
    ...
    handler: async (args) => {
      const { workspacePath } = ws(ctx, args);
      const state = await ctx.runtime.callForWorkspace<Record<string, unknown>>("workspace/readState", workspacePath, {}, 60_000);
      const revision = Number(state?.revision ?? (state?.modelCatalog as Record<string, unknown> | undefined)?.revision ?? 0);
  ...
  tools.push({
    ...
    handler: async (args) => {
      ...
      if (modelRef) {
        const state = await ctx.runtime.callForWorkspace<{ modelCatalog?: { available?: Array<Record<string, unknown>> } }>("workspace/readState", workspacePath, {});
