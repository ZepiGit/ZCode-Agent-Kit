/**
 * MCP tool definitions. JSON Schema driven (low-level SDK Server), with
 * structured results plus a text fallback, redaction, workspace allowlist
 * enforcement and read-only guards.
 */
import type { RuntimeManager } from "../runtime/manager.js";
import type { TaskManager, TaskState, TaskResultV1 } from "../tasks/manager.js";
...
export interface ToolSpec {
  ...
  /** When true the tool changes state and is blocked in read-only mode. */
  mutating: boolean;
...
function str(args: Record<string, unknown>, key: string, required = true): string {
  ...
  if (required) throw new Error(`missing or invalid argument: ${key} (string expected)`);
...
function optBool(args: Record<string, unknown>, key: string): boolean | null {
  const v = args[key];
  return typeof v === "boolean" ? v : null;
}

function optObj(args: Record<string, unknown>, key: string): Record<string, unknown> | null {
  const v = args[key];
  return v !== null && typeof v === "object" && !Array.isArray(v) ? (v as Record<string, unknown>) : null;
}

/** Resolve + allowlist-check a workspace path argument. */
function ws(ctx: ToolContext, args: Record<string, unknown>): { workspacePath: string; workspaceKey: string } {
...
function requireWritable(ctx: ToolContext, tool: string): void {
...
function parseModelRef(args: Record<string, unknown>): { providerId: string; modelId: string } | null {
  ...
  if (s && s.includes("/")) {
    ...
    if (slash === 0 || slash === s.length - 1) throw new Error("model must contain a nonempty providerId and modelId");
    return { providerId: s.slice(0, slash), modelId: s.slice(slash + 1) };
  }
  if (args.model !== undefined) throw new Error("model must be providerId/modelId");
  return null;
}
...
export function buildTools(ctx: ToolContext): ToolSpec[] {
  ...
  tools.push({
    ...
    description:
      "Bridge + harness health: runtime detection result, harness version and bundle fingerprint, protocol, process state, bridge uptime, read-only mode, allowlisted workspaces.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
    mutating: false,
    handler: async () => {
      const d = ctx.runtime.diagnostics() as Record<string, unknown>;
  ...
  tools.push({
    ...
    handler: async (args) => {
      ...
      if (caps.length === 0) return { method, known: false, note: "not in registry; zod validation errors from the harness define live schemas" };
  ...
  tools.push({
    ...
    description:
      "Select model (providerId/modelId), reasoning and/or permission mode for a session. Returns requested and effective values with actual read-back comparison. Native 0.16.9 has no workspace-default setters; scope=workspace returns an explicit unsupported error without mutation.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    handler: async (args) => {
      ...
      if (scope === "session") {
        ...
        if (modelRef) mutations.push(["session/setModel", { model: { ...modelRef, ...(thoughtLevel === null ? {} : { options: { reasoningLevel: thoughtLevel } }) } }]);
        else if (thoughtLevel !== null) mutations.push(["session/setThoughtLevel", { thoughtLevel }]);
        if (mode) mutations.push(["session/setMode", { mode }]);
        if (!mutations.length) throw new Error("nothing to set: provide model, thoughtLevel or mode");
        for (const [method, fields] of mutations) {
          const result = await ctx.runtime.call<Record<string, unknown>>(method, { sessionId, ...fields, ...(revision === null ? {} : { expectedRevision: revision }) });
          if (revision !== null) {
            const next = (result.runtime as Record<string, unknown> | undefined)?.stateRevision;
            if (typeof next !== "number" || !Number.isSafeInteger(next) || next < 0) throw new Error("INVALID_NATIVE_RESPONSE: setter omitted session revision; refusing remaining mutations");
            revision = next;
          }
        }
        ...
        const sMode = (settings.mode ?? {}) as Record<string, unknown>;
        const sThought = (settings.thoughtLevel ?? settings.reasoning ?? {}) as Record<string, unknown>;
        ...
        return {
          ...
          revision: (readBack.runtime as Record<string, unknown> | undefined)?.stateRevision ?? null,
      ...
      if (modelRef) changes.model = `${modelRef.providerId}/${modelRef.modelId}`;
      if (mode) changes.mode = mode;
      if (thoughtLevel) changes.thoughtLevel = thoughtLevel;
      if (Object.keys(changes).length === 0) throw new Error("nothing to set: provide model, thoughtLevel or mode");
  ...
  tools.push({
    ...
    handler: async (args) => {
      ...
      if (scope === "desktop") return ctx.settings.desktopInventory();
      const { workspacePath } = ws(ctx, args);
  ...
  tools.push({
    ...
    handler: async (args) => {
      ...
      if (modelRef) {
        const state = await ctx.runtime.readWorkspaceCatalog(workspacePath);
        const available = (state.modelCatalog as { available: Array<Record<string, unknown>> }).available;
        ...
        if (!known) {
          throw new Error(
            `UNKNOWN_MODEL: ${modelRef.providerId}/${modelRef.modelId} is not in the live catalog. Available: ${safeJsonStringify(
              available.map((m) => {
                const ref = (m.ref ?? {}) as Record<string, unknown>;
  ...
  tools.push({
    ...
    handler: async (args) => {
      ...
      if (!rec) throw new Error(`unknown task: ${String(args.taskId)}`);
      return rec;
    },
  });
  ...
  tools.push({
    ...
    handler: async (args) => {
      const wp = optStr(args, "workspacePath");
      const state = optStr(args, "state") as TaskState | null;
      return {
  ...
  tools.push({
    ...
    mutating: false,
    handler: async (args) => {
      const result: TaskResultV1 = await ctx.tasks.buildResult(str(args, "taskId"));
      return result;
    },
  });
...
export function toolTextPayload(value: unknown): string {
