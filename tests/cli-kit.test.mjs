...
// F11: omp 18+ no longer evaluates the `!node` YAML tag — the managed block
// must carry a LITERAL key, and verify() must flag stale/mismatched keys
// (copy switches, rotations) so `zcode-kit setup` repairs them.
test("omp adapter writes literal key (omp 18+ ignores !node), self-checks staleness, integrates repair it", async () => {
  const omp = (await import("../cli/adapters/omp.mjs")).default;
  const { ctx } = ctxWithAppData(fakeHome("omp-literal"));
  const agentDir = join(ctx.home, ".omp", "agent");
  mkdirSync(agentDir, { recursive: true });
  writeFileSync(
    join(agentDir, "models.yml"),
    `providers:\n# >>> zcode-kit (managed block) — do not edit inside\n  zcode:\n    name: ZCode\n    apiKey: "stale-key-from-another-copy-Aaa111"\n# <<< zcode-kit\n`,
  );
  try {
    const before = omp.verify(ctx).find((c) => c.name === "omp key resolver");
    assert.equal(before?.ok, false, "verify must flag a stale/mismatched key");
    assert.match(String(before?.detail ?? ""), /rerun zcode-kit setup/);
    const res = omp.apply(ctx, noopTx(), () => {});
    assert.equal(res.changed, true, "integrate must repair the block");
    const written = readFileSync(join(agentDir, "models.yml"), "utf8");
    assert.doesNotMatch(written, /!node/, "omp 18+ ignores !node — block must use a literal key");
    assert.ok(written.includes(`apiKey: "${ctx.key()}"`), "block must embed this copy's current key");
